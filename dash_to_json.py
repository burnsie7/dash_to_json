...
def dash_to_json(dash, board_type, filename):
    ...
    if board_type == "timeboard":
        ...
        # TODO: API doesn't accept process graphs or log and apm datasets.
        for graph in graphs:
            add_graph = True
            viz = None
            rs = []
            if graph.get('definition', None):
                viz = graph['definition'].get('viz', None)
                rs = graph['definition'].get('requests', None)
            elif graph.get('tile_def', None):
                viz = graph['tile_def'].get('viz', None)
                rs = graph['tile_def'].get('requests', None)
            if viz == 'process':
                add_graph = False
            for r in rs:
